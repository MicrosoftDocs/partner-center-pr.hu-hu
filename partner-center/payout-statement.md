...
- **Az idei év bevételei** – A kifizetett és a következő hónapban fizetendő bevételek teljes összege és lebontása.
...
- **Becsült kifizetési hónap** – A bevétel várható kifizetésének hónapja. További [információért tekintse meg a](#estimated-payment-month) következő szakaszt.
...
- Módosítások: A tényleges összeg az eltért vagy elküldött módosításoktól függően változik.
...
| earningRate (bevételi arány) | A bevételt generáló tranzakció összegére alkalmazott ösztönzők | Mind |
| earningType (bevétel típusa) | Jelzi, hogy díjról, költségről, együttműködésről, értékesítésről és így tovább | Mind |
| exchangeRateDate | Exchange usd bevétel kiszámításához használt árfolyamdátum kiszámítása | Mind |
| externalReferenceId (külsőreferenceId) | A program egyedi azonosítója | Közvetlen fizetési programok (ösztönzők és piacterek) |
| externalReferenceIdLabel | Egyedi azonosítócímke | Közvetlen fizetési programok (ösztönzők és piacterek) |
| instantRebateAmount |  |  |
| invoiceDate (számla dátuma) |  |  |
| invoiceNumber (számlaszám) | Számlaszám (csak nagyvállalatok számára alkalmazható) | Ösztönzők és piacterek – csak néhány program |
| lastPaymentCurrency (lastPaymentCurrency) | Utolsó fizetési pénznem (a mező üres lesz, ha még nem történt kifizetés) |  |
| Kar | A bevétel üzleti szabályát jelzi | Mind |
| LicensingProgramName | A licencprogram neve |  |
...
| paymentId (fizetésiazonosító) | Egyedi azonosító, amely a tranzakciós jelentésben az összes tranzakciót a fizetési jelentésben megadott kifizetéssel korrelálja | Mind |
| paymentStatus | Fizetési állapot | Mind |
| paymentStatusDescription (paymentStatusDescription) | A fizetési állapot rövid leírása | Mind |
...
| storeFee | A Microsoft által az alkalmazás vagy bővítmény Áruházban való elérhetővé fizetéseként megőrzhető összeg | Piacokon |
...
| taxCity (adóváros) |  |  |
| taxCountry (ország) |  |  |
| taxRemitModel | Adók (értékesítés, felhasználás vagy áfa-/GST-adók) átutalásáért felelős fél | Piacokon |
| taxRemitted (visszaadó) | A kivetített adó mennyisége (értékesítés, felhasználás vagy áfa-/GST-adó) | Piacokon |
| taxState (taxState) | Az ügyfél állapota |  |
| taxZipCode | Ügyfél irányítószáma |  |
| tpan (tpan) | A külső hirdetéshálózatot jelzi | marketplaces – csak hirdetések |
| transactionAmount (tranzakció összege) | A tranzakció összege az eredeti tranzakciós pénznemben, amely alapján a bevétel létre lesz hozva | Mind |
...
| transactionDate (tranzakció dátuma) | A tranzakció dátuma. Olyan programokhoz hasznos, amelyekben több tranzakció is hozzájárul egy bevételhez | Mind |
| transactionExchangeRate | Exchange usd összegnek megfelelő tranzakció díjdátumának megjelenítése | Mind |
| transactionId (tranzakcióazonosító) | A tranzakció egyedi azonosítója | Mind |
| transactionPaymentMethod | A tranzakcióhoz használt ügyfél fizetési eszköze, például kártya, mobilszolgáltatói számlázás vagy PayPal | Piacokon |
...
| Együttműködésváltás | Az együttműködési bevételek másik időszakra való átcsoportosítását, vagy az együttműködési bevételeket átalakításával. |
...
| Partnerleképezés (előfizetés) MCI/CSP | Az előfizetések hibás minálást kijavító módosítása. |
...
>[!Note]
> Ahhoz, hogy jogosult legyen a kifizetésre, a bevételnek el kell érnie az 50 USD fizetési küszöbértéket. [](payment-thresholds-methods-timeframes.md) További információ: Microsoft Publisher [szerződés.](/legal/marketplace/msft-publisher-agreement)

:::image type="content" source="images/payouts/payments-overview.png" alt-text="Kifizetések áttekintése képernyő.":::

- **Teljes kifizetés ebben az évben** – Az ebben az évben kifizetett teljes összeg, amerikai dollárban az összes programért.
...
- **Kifizetés forrás szerint** – A kifizetések összege (amerikai dollárban), programonként, az elmúlt 12 hónapban.
...
- **Fizetős** – Minden sikeresen elküldött kifizetés. Válassza ki az évet a legördülő menüben az adott évben kiadott kifizetések szűréséhez.
...
| --- | --- | --- |
| Feldolgozatlan | A bevétel jogosult a kifizetésre. Ebben az állapotban marad egy hűtési időszakra vonatkozóan, amint azt a program útmutatója Ösztönzőprogram. | No |
...
| paymentDate (kifizetésidátum) | A Microsoft által küldött fizetés dátuma |
|
...
| Mennyiség | Ha a Revenue Source (Bevétel forrása) Microsoft Store Vállalatoknak, a Quantity (Mennyiség) a megvásárolt licencek számát jelöli. Minden más bevételi forrás esetén a Mennyiség mindig 1 lesz. Még ha egyetlen tranzakció is két sorelemre van felosztva, mert két különböző fizetési módot használt, minden sortétel 1 Quantity (Mennyiség) elemet fog mutatni. |
...
| Adó újrafizetése | A kivetített adó mennyisége (értékesítés, felhasználás vagy áfa/GST-adók) |
...
| Adókat visszatartott | A visszatartott bevételi adó összege (a fenntartott **CSV-fájlban** nem szerepel) |
| Payment | Az alkalmazás kevesebb alkalmazható bevételi adóelőleget (a tranzakciós pénznemben látható összeget) használja. A Reserved **CSV-fájl** nem tartalmazza. |
