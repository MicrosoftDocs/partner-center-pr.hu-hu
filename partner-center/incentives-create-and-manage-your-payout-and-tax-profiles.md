...
>[!IMPORTANT]
>A fizetési és adóprofilok létrehozásához vagy kezeléséhez ösztönzőrendszergazdának kell lennie a Partnerközpont. Az ösztönzői szerepköröket minden egyes ösztönzőprogramban minden MPN-helyhez hozzá kell rendelni. További információ az ösztönzőrendszergazdák hozzáadásáról a Partnerközpont: [Felhasználói fiókok létrehozása.](create-user-accounts-and-set-permissions.md)
...
2. A fogaskerék ikonra kattintva nyissa meg **Gépház** menüt, majd válassza a **Fiókbeállítások lehetőséget.**
...
1. Jelentkezzen be a [Partnerközpont irányítópultjára,](https://partner.microsoft.com/dashboard/)majd válassza a fogaskerék ikont a Gépház **megnyitásához.** 
...
4. Válassza **a Fizetési mód lehetőséget.**
   ...
   - Ha fizetési módként a **Credit Note** (Jóváírás) beállítást választotta, akkor töltse ki az ellenőrzést. Ez megerősíti, hogy a hivatkozott SAP-szám a szervezethez tartozik.
    ...
    - Ha a hely MPN-azonosítóját a Microsoft helyi leányvállalata fizeti ki egy adott ösztönzőprogramért, és engedélyezi az LRD (korlátozott kockázat terjesztője) jóváírási emlékeztetőt fizetési módként, akkor a fizetési profilja előre ki lesz töltve az LRD Credit Note fizetési mód használatával. A megfelelő ösztönzőprogramhoz és hely MPN-azonosítóhoz tartozó LRD-kreditjegy  fizetési mód sorában a Megerősített vagy Az ellenőrzés szükséges állapotot fogja látni a fizetési profil szakaszban. 
      ...
      Ha az állapot **Megerősített,** akkor a CSP-bérlőazonosító hozzárendelése befejeződött, és nincs szükség további műveletre. A hozzárendelés részleteinek megtekintése továbbra is a Megerősített lehetőséget választhatja.
   
      Az olyan országokban, amelyekben a partnerek kifejezetten kérik az adómentesség kérelmezését, lehetőség van az adómentesség alkalmazásra az adóprofil mellett az ösztönzőprogram és a hely MPN adóprofil szakaszában. Ebben a mezőben az ösztönző jóváírási megjegyzésére alkalmazva lesznek az adómentességi kedvezmények. 
   
      Az LRD Credit Note fizetési mód jelenleg csak Ausztrália, Új-Zéland és Kanada partnerek számára érhető el a Microsoft Commerce Incentive programhoz. Ha Ön közvetlen számlázási partner vagy közvetett szolgáltató abban a három országban, amely regisztrálva van az MCI-programra, és nem látja az LRD-kreditek megjegyzését elérhető fizetési módként, ellenőrizze, hogy a bérlőazonosító társítva van-e a megfelelő partner MPN-helyfiókkal. Erről a szervezeti profil frissítésével kapcsolatos további információkért olvassa el a [következőt:](update-your-partner-profile.md).
...
Beállíthat alapértelmezett bankprofilokat, és hozzárendelheti őket MPN-helyekhez. A Microsoft ezeket az alapértelmezett profilokat fogja használni az adott MPN-helyhez történő, ezt követő regisztrációkhoz. 

1. Jelentkezzen be a [Partnerközpont irányítópultra,](https://partner.microsoft.com/dashboard/)majd válassza a **** fogaskerék ikont a Gépház   megnyitásához. 
...
5. Válasszon ki egy banki profilt a vállalata elérhető banki profiljainak listájából, válassza ki a banki profillal használni kívánt pénznemet, majd válassza ki azon MPN-helyek listáját, amelyekre alkalmazni szeretné ezt az alapértelmezett profilt.
...
A banki profilok vállalati szinten vannak létrehozva. Ez lehetővé teszi, hogy egy banki profil több MPN-azonosítóhoz és ösztönzőprogramhoz legyen hozzárendelve egy vállalaton belül. Előfordulhatnak kivételek, amikor a banki profilt különböző országokra alkalmazza, mivel különböző banki és adózási szabályok alkalmazhatók.
...
A rendszer átirányítja a Kifizetési **és adóprofilok oldalra.** Az új profil állapota a **Microsoft-ellenőrzés** függőben lévő állapotát fogja tükrözni, amíg az érvényesítés be nem fejeződött. Ez a folyamat akár 48 órát is igénybe vehet. Az érvényesítés befejezése után a profil  állapota Jóváhagyás vagy **Beavatkozás szükséges** lesz. Ha **beavatkozás szükséges,** ismételje meg a fenti lépéseket a szükséges információk megismétlve. 
...
Az amerikai partnervállalatok esetében, ha információra van szüksége a W8- vagy W9-űrlapok kitöltésével kapcsolatban, a következő címek írják le az IRS webhelyét:
...
2. A Beállítás **lapon** válassza ki a vállalatára vonatkozó beállítást.
   ...
   - Válassza a jobb oldalon található lehetőséget, ha a vállalata az Egyesült Államok kívülre van beépítve, majd válassza ki országát/régióját a listából.

3. Kattintson a **Tovább** gombra. 

4. Az Adó **állapota lapon** adja meg a szükséges adatokat, majd válassza a Tovább **lehetőséget.** Az oldal mezői országonként változnak. a részleteket. 
...
1. Az adózási űrlap szakaszban frissítse az adózási űrlapokat az új adatok megszűrével. 

## <a name="next-steps"></a>Következő lépések

- [A kifizetési és adókkal kapcsolatos gyakori kérdések](payout-faq.yml)
